...
var NatsServer = function (serviceName, serviceId, routes, natsConfig) {
	var self = this;
	Server.call(this, serviceName, serviceId, routes);
...
NatsServer.prototype.createServiceRoutes = function () {
	var self = this;
	var queue = new Queue(self.serviceName, self.config.connection);
	queue.process(self.requestHandler.bind(self));
	self.serviceRoutesQueue = queue;
};


NatsServer.prototype.createSpecificServiceRoutes = function () {
	var self = this;
	var queue = new Queue(self.serviceName + ":" + self.serviceId, self.config.connection);
	queue.process(self.requestHandler.bind(self));
	self.specificServiceRoutesQueue = queue;
};
